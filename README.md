...
| `acp check --json [--evidence acp-evidence.json]` → authority, current test traces, committed fixtures | Runs authority and **all incident invariants against current observed events**, not historical events | JSON summary includes the ACP version and deterministic authority-contract SHA-256; every source also includes a SHA-256 of its normalized observed events. `--evidence` saves that exact report in an `acp-check-evidence/v1` envelope with a canonical report SHA-256 and refuses to overwrite an existing record. 0/2/3/4 below. |
| `acp verify-evidence acp-evidence.json [--json]` | Recomputes the canonical report SHA-256 from a saved evidence pack | Prints `VALID` (or the verified embedded report) and exits 0 only when schema and hash match; malformed, unsupported or tampered packs exit 4. This proves file integrity, not signer identity or trace completeness. |
